        ...
        result.map do |row|
          ...
          if ::Rails.version >= '4'
            unless column_names.empty?
              ...
              ::ActiveRecord::ConnectionAdapters::PostgreSQLAdapter::IndexDefinition.new(table_name, index_name, unique, column_names, [], orders, where, nil, using)
            end
          else
            # add info on sort order for columns (only desc order is explicitly specified, asc is the default)
            desc_order_columns = inddef.scan(/(\w+) DESC/).flatten
            ...
            column_names.empty? ? nil : ::ActiveRecord::ConnectionAdapters::PostgreSQLAdapter::IndexDefinition.new(table_name, index_name, unique, column_names, [], orders)
          end
        end.compact
      end
